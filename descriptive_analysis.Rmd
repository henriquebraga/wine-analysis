...
```{r split_wine_datasets}
```


Maiores valores de álcool por tipo de vinho (tinto ou branco);

```{r aggregate_wines_dt_alcohol}
aggregate(wines_dt$alcohol, by = list(wines_dt$Vinho), FUN=mean, na.rm = TRUE)
```
...
Podemos notar que existe uma grande diferença nos campos para cada tipo de vinho. A acidez, por exemplo, varia praticamente o dobro.
